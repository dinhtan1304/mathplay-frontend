...
import { questionsApi, bankExportApi } from '@/lib/api'
...
// ─── Question Card ────────────────────────────────────────────────────────────
function QuestionCard({ q, idx, selected, onSelect, onEdit, onDelete, deleting }: {
  q: Question; idx: number; selected: boolean
    ...
    <div className={cn('group border-b border-bg-border last:border-0 transition-colors', selected ? 'bg-accent/5' : 'hover:bg-bg-hover/40')}>
      <div className="flex items-start gap-0">
        ...
        <div className="flex-1 min-w-0 py-3.5 pr-2">
          ...
          <p className="text-[10px] text-text-dim mt-2">{formatDateTime(q.created_at)}</p>
        </div>
...
export default function BankPage() {
  ...
  const loadQuestions = useCallback(async () => {
    setLoading(true)
    try {
      const res = await questionsApi.list({ page, page_size: pageSize, type: filterType || undefined, difficulty: filterDiff || undefined, topic: filterTopic || undefined, keyword: debouncedSearch || undefined })
  ...
  const exportSelected = async (format: 'docx' | 'pdf' | 'latex') => {
    setExporting(true)
    const params = { topic: filterTopic || undefined, difficulty: filterDiff || undefined, question_type: filterType || undefined, keyword: search || undefined }
    try {
      if (format === 'pdf') {
  ...
  const hasActiveFilter = !!(filterType || filterDiff || filterTopic || search)
      ...
      <div className="px-6 pt-6 pb-4 flex items-start justify-between gap-4 flex-shrink-0">
        <div>
          ...
          <p className="text-text-muted text-sm mt-0.5">
            ...
            {hasActiveFilter && <span className="text-accent ml-1">· đang lọc</span>}
          </p>
        </div>
