...
export default function VanillaTemplate({
  ...
  const { teacher, colors, contact, sections, courses, exams, schedule } =
    config;
  const p = colors.primary;
  const allQuizzes = (quizInfo ?? []).filter((q) => !q.not_found);
  const visCourses = slug
    ? courses.slice(0, SECTION_THRESHOLDS.courses)
    : courses;
  const visExams = slug ? exams.slice(0, SECTION_THRESHOLDS.exams) : exams;
  const visSchedule = slug
    ? schedule.slice(0, SECTION_THRESHOLDS.schedule)
    : schedule;
  const quizzes = slug
    ? allQuizzes.slice(0, SECTION_THRESHOLDS.quiz)
    : allQuizzes;
  ...
  const navItems: {
    ...
    {
      id: "courses" as SectionId,
      label: "Khóa học",
      icon: <BookOpen className="w-4 h-4" />,
      show: !!(sections.courses && courses.length > 0),
    },
    {
      id: "exams" as SectionId,
      label: "Bộ đề",
      icon: <FileText className="w-4 h-4" />,
      show: !!(sections.exams && exams.length > 0),
    },
    {
      id: "quizzes" as SectionId,
      label: "Quiz",
      icon: <Gamepad2 className="w-4 h-4" />,
      show: !!(sections.quizzes && allQuizzes.length > 0),
    },
    {
      id: "schedule" as SectionId,
      label: "Lịch học",
      icon: <Calendar className="w-4 h-4" />,
      show: !!(sections.schedule && schedule.length > 0),
    },
    {
  ...
  return (
    ...
    >
      <div className="max-w-5xl mx-auto flex">
        ...
        <aside className="hidden md:flex flex-col gap-1 sticky top-0 h-screen w-52 shrink-0 py-8 pr-4 pl-5 overflow-y-auto">
          ...
          <div className="mb-6 text-center">
            {teacher.avatar ? (
              ...
              <div
                className="w-14 h-14 rounded-full mx-auto mb-2 flex items-center justify-center font-bold text-white text-lg"
                style={{ background: p }}
              >
            ...
            <p className="text-xs mt-0.5" style={{ color: p }}>
              {teacher.subject}
            </p>
          </div>
          ...
          {navItems.map((n) => (
            ...
            >
              <span style={{ color: active === n.id ? p : "#d97706" }}>
                {n.icon}
              </span>
              {n.label}
            </button>
        ...
        <main
          ref={contentRef}
          className="flex-1 min-w-0 px-5 sm:px-8 py-10 space-y-16"
        >
          {/* About */}
          <section id="section-about">
            <div className="flex items-start gap-5 mb-6">
              {teacher.avatar ? (
                ...
                <div
                  className="w-20 h-20 rounded-2xl flex items-center justify-center text-2xl font-bold text-white md:hidden shrink-0"
                  style={{ background: p }}
                >
                  {teacher.name.charAt(0)}
                </div>
            ...
            >
              <p className="text-amber-800 leading-relaxed italic text-base mb-4">
            ...
            <section id="section-courses">
              ...
              <div className="space-y-3">
                {visCourses.map((c, i) => (
                  <div
                    key={i}
                    className="bg-white rounded-3xl p-5 flex gap-4"
                    style={{ border: "1px solid #fef08a" }}
                  >
                    <div
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-bold text-amber-900 mb-1 leading-tight">
                        {c.name}
                      </h3>
                      <p className="text-xs mb-1.5" style={{ color: p }}>
                        {c.schedule}
                      </p>
              ...
              <SeeMoreLink
                ...
                threshold={SECTION_THRESHOLDS.courses}
                color={p}
              />
            </section>
          )}
          ...
          {sections.exams && exams.length > 0 && (
            <section id="section-exams">
              ...
              <SeeMoreLink
                ...
                section="exams"
                total={exams.length}
                threshold={SECTION_THRESHOLDS.exams}
                color={p}
              />
            </section>
          )}
          ...
          {sections.quizzes && allQuizzes.length > 0 && (
            <section id="section-quizzes">
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {quizzes.map((q) => (
                  ...
                  >
                    ...
                    <span
                      className="text-xs font-bold flex items-center gap-1 mt-auto group-hover:translate-x-0.5 transition"
                      style={{ color: p }}
                    >
              ...
              <SeeMoreLink
                ...
                threshold={SECTION_THRESHOLDS.quiz}
                color={p}
              />
            </section>
          )}
          ...
          {sections.schedule && schedule.length > 0 && (
            <section id="section-schedule">
              ...
              <div
                className="bg-white rounded-3xl divide-y divide-amber-100 overflow-hidden"
                style={{ border: "1px solid #fef08a" }}
              >
                {visSchedule.map((s, i) => (
                  <div key={i} className="flex items-center gap-5 px-5 py-3.5">
                    <span className="font-bold text-sm text-amber-900 w-24 shrink-0">
                      {s.day}
                    </span>
                    <span
                    ...
                    <span className="text-xs text-amber-700 flex-1 truncate">
                      {s.subject}
                    </span>
                  </div>
                ))}
              ...
              <SeeMoreLink
                ...
                threshold={SECTION_THRESHOLDS.schedule}
                color={p}
              />
            </section>
          )}
            ...
            <section id="section-contact">
              ...
              >
                <p className="text-amber-700 text-sm mb-4">
                  Liên hệ để nhận tư vấn và xếp lớp phù hợp
                </p>
                {contact.email && (
                  <a
                    href={`mailto:${contact.email}`}
                    className="flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium text-white transition hover:opacity-90"
                    style={{ background: p }}
                  >
                ...
                {contact.zalo && (
                  <a
                    ...
                    style={{ background: `${p}15`, color: p }}
                  >
                    <ExternalLink className="w-4 h-4 shrink-0" /> Zalo:{" "}
                    {contact.zalo}
                  </a>
                )}
              </div>
