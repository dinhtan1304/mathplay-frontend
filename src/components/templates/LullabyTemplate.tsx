...
export default function LullabyTemplate({
  config,
  quizInfo,
  slug,
}: TemplateProps) {
  ...
  const cards: StoryCard[] = [
    {
      ...
      content: (
        <div className="flex flex-col items-center text-center h-full justify-center px-6">
          ...
          <p className="text-xs font-bold uppercase tracking-[0.3em] mb-2 opacity-70">
            {teacher.subject}
          </p>
              ...
              <div className="flex flex-col h-full px-7 py-8">
                ...
                <div className="space-y-2 flex-1 overflow-y-auto">
                  {visSchedule.map((s, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-3 bg-white/20 rounded-xl px-4 py-2.5 backdrop-blur-sm"
                    >
                      <div className="text-xs font-bold w-20 shrink-0">
                        {s.day}
                      </div>
                      <div className="text-xs font-mono opacity-80">
                        {s.time}
                      </div>
                      <div className="text-xs flex-1 truncate opacity-90">
                        {s.subject}
                      </div>
                    </div>
                  ))}
            ...
            content: (
              <div className="flex flex-col h-full px-7 py-8 justify-center items-center text-center">
                ...
                <p className="text-sm opacity-75 mb-6">
                  Liên hệ ngay để xếp lớp phù hợp
                </p>
                <div className="space-y-3 w-full max-w-xs">
                  {contact.email && (
                    <a
                      href={`mailto:${contact.email}`}
                      className="flex items-center justify-center gap-2 bg-white/25 hover:bg-white/35 transition rounded-2xl py-3 text-sm font-medium backdrop-blur-sm"
                    >
  ...
  return (
    <div
      className="min-h-screen font-sans flex flex-col items-center justify-center"
      style={{
      ...
      <div className="w-full max-w-sm mx-auto px-4 py-8">
        ...
        <div className="flex gap-1 mb-3">
          {cards.map((_, i) => (
            <div
              key={i}
              className="h-0.5 flex-1 rounded-full overflow-hidden bg-white/40 cursor-pointer"
              onClick={() => setActiveCard(i)}
            >
              <div
                className="h-full transition-all duration-300 rounded-full"
                style={{
                  width: i <= activeCard ? "100%" : "0%",
        ...
        <div className="flex items-center justify-between mt-5">
          ...
          <button
            onClick={() =>
              setActiveCard((i) => Math.min(cards.length - 1, i + 1))
            }
          ...
          {cards.map((card, i) => (
            <button
              ...
              style={{
                ...
                outline: i === activeCard ? `2px solid ${card.accent}` : "none",
